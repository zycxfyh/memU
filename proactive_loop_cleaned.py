...
class ProactiveMemoryLoop:
    ...
    def load_state(self):
        ...
        if self.state_file.exists():
            try:
                ...
                print(f"[DISK] Loaded previous state - Last scan: {self.last_scan_time}")
    ...
    async def scan_openclaw_activity(self) -> List[Dict[str, Any]]:
        ...
        print("[SCAN] Scanning for new OpenClaw activity...")
        ...
        if diary_dir.exists():
            for diary_file in diary_dir.glob("*.md"):
                ...
                if mod_time > self.last_scan_time:
                    print(f"  [EMOJI] Found updated diary: {diary_file.name}")
                    ...
                    try:
                        ...
                        print(f"  [EMOJI]  Could not read {diary_file}: {e}")
        ...
        if logs_dir.exists():
            for log_file in logs_dir.glob("*.log"):
                ...
                if mod_time > self.last_scan_time:
                    print(f"  [EMOJI] Found updated log: {log_file.name}")
                    ...
                    try:
                        ...
                        print(f"  [EMOJI]  Could not read {log_file}: {e}")
    ...
    async def extract_memories(self, activities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        print("[MEM] Extracting memories from detected activities...")
        ...
        for activity in activities:
            try:
                ...
                print(f"  [EMOJI]  Error extracting memories from {activity['source']}: {e}")
    ...
    async def consolidate_memories(self):
        ...
        print("[CONSOL] Consolidating memories...")
        ...
        try:
            ...
            print(f"  [EMOJI]  Error during consolidation: {e}")

    async def run_single_iteration(self):
        """Run one iteration of the proactive memory loop"""
        print(f"\n[LOOP] Starting proactive memory iteration at {datetime.now()}")
        ...
        try:
            ...
            if new_activities:
                ...
                if extracted:
                    print(f"  [EMOJI] Successfully extracted {len(extracted)} new memories!")
                    
                    # Update the last scan time to now
                    self.last_scan_time = datetime.now()
                else:
                    print("  [EMOJI]  Activities found but no specific memories extracted")
            else:
                print("  [EMOJI]  No new activity detected since last scan")
            ...
            print(f"[EMOJI] Iteration completed at {datetime.now()}")
            
        except Exception as e:
            print(f"[EMOJI] Error in proactive memory iteration: {e}")
            import traceback
            traceback.print_exc()

    async def run_continuous(self):
        """Run the proactive memory loop continuously"""
        print("[START] Starting continuous proactive memory loop...")
        ...
        try:
            ...
            print("\n[EMOJI] Received interrupt signal")
        finally:
            self.running = False
            print("[STOP] Proactive memory loop stopped")
...
async def main():
    ...
    print("[MEM] MemU + OpenClaw Proactive Memory System")
    print("="*50)
